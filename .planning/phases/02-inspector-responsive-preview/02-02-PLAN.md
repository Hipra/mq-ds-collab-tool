...
- For complex props (rawType === 'expression' with value `'{...}'` or `'[N items]'` or `'() => ...'`): show the summary inline. On click, toggle expansion (show `'{...}'` or expand — but since we only have the summary from AST, expansion just shows the summary text. Full expansion would require re-parsing which is out of scope. Just show the summary as-is, no expansion needed for v1).
- Keep it simple: just render the PropEntry[] as a vertical list of name/value pairs.
    ...
    <Box sx={{ p: 1 }}>
      ...
      {node.props.length === 0 ? (
        ...
        <Box component="table" sx={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'monospace', fontSize: '12px' }}>
          <tbody>
            {node.props.map((prop) => (
              <Box component="tr" key={prop.name} sx={{ '&:hover': { bgcolor: 'action.hover' } }}>
...
4. Click a component in the tree — PropInspector shows its props with values and source location
